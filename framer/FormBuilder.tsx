    ...
    const renderField = (field: FieldConfig, index: number) => {
        ...
        switch (type) {
            ...
            case "radio":
                input = (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {optionsList.map((opt) => (
                            <label key={opt} style={{
                                fontFamily: font, fontSize, color: optionColor, display: "flex", alignItems: "center",
                                gap: 8, cursor: "pointer"
                            }}>
                                <input type="radio" name={name} value={opt} checked={val === opt} onChange={(e) => handleChange(name,
                                    e.target.value, type)} style={{ accentColor: focusBorderColor }} />
                                {opt}
                            </label>
                        ))}
                    </div>
            ...
            case "checkbox":
                input = (
                    <label style={{
                        fontFamily: font, fontSize, color: optionColor, display: "flex", alignItems: "center", gap: 8, cursor:
                            "pointer"
                    }}>
                        <input type="checkbox" checked={val === "true"} onChange={(e) => handleChange(name, String(e.target.checked), type)}
            ...
            case "currency":
                input = <input type="text" inputMode="numeric" style={inputStyle} placeholder={placeholder || "Ex: 100.000,00"}
                    required={required} value={val} onChange={(e) => handleChange(name, e.target.value, type)} />
                break

            case "tel":
                input = <input type="tel" inputMode="numeric" style={inputStyle} placeholder={placeholder || "(00) 0000-0000"}
                    required={required} value={val} onChange={(e) => handleChange(name, e.target.value, type)} />
                break

            default:
                input = <input type={type} style={inputStyle} placeholder={placeholder} required={required} value={val} onChange={(e) =>
...
FormBuilder.defaultProps = {
    ...
    fields: [
        { label: "Nome", name: "name", type: "text", placeholder: "Seu nome completo", required: true, options: "", halfWidth: true },
        {
            label: "E-mail Corporativo", name: "email", type: "email", placeholder: "johndoe@minhaempresa.com.br", required: true, options: "",
            halfWidth: true
        },
        {
            label: "Telefone / WhatsApp", name: "phone", type: "tel", placeholder: "(00) 00000-0000", required: true, options: "", halfWidth:
                true
        },
        {
        ...
        {
            label: "Site", name: "site", type: "url", placeholder: "https://meusite.com.br", required: false, options: "", halfWidth: true
        },
        {
            label: "Qual o faturamento mensal da sua empresa?", name: "revenue", type: "select", placeholder: "Selecione...", required: true,
            options: "0 a 99 mil; 100 mil a 499 mil; 500 mil a 999 mil; 1 milhao a 7 milhoes; Acima de 8 milhoes",                                                    
        },
        {
            label: "Qual plataforma você utiliza no seu e-commerce?", name: "platform", type: "select", placeholder: "Selecione...", required:
                true, options: "Não possuo, Shopify, VTEX, Outra", halfWidth: true
        },
        {
            label: "Cargo", name: "role", type: "select", placeholder: "Selecione...", required: true, options: "Diretor, Proprietário, Gerente, Coordenador, Analista, Assistente", halfWidth: true
        },
        {
            label: "Número de funcionários", name: "employees", type: "select", placeholder: "Selecione...", required: true, options: "1 - 5, 6 - 10, 11 - 20, 21 - 50, 51 - 100, Mais de 100", halfWidth: true
        },
        {
            label: "Como você conheceu a Wave Commerce?", name: "source", type: "select", placeholder: "Selecione...", required: true, options:
                "Indicação, Instagram, LinkedIn, Google, Outro", halfWidth: true
        },
        {
            label: "Como podemos te ajudar?", name: "message", type: "textarea", placeholder: "Descreva seu projeto ou necessidade...", required: false, options: "", halfWidth: false
        },
        {
...
addPropertyControls(FormBuilder, {
    ...
    fontWeight: {
        type: ControlType.Number, title: "Peso fonte", min: 100, max: 900, step: 100, defaultValue:
            FormBuilder.defaultProps.fontWeight
    },
    fontStyle: {
        type: ControlType.Enum,
        title: "Estilo fonte",
    ...
    labelSize: {
        type: ControlType.Number, title: "Tamanho label", min: 8, max: 18, step: 1, defaultValue: FormBuilder.defaultProps.labelSize
    },
    labelWeight: {
        type: ControlType.Number, title: "Peso label", min: 300, max: 900, step: 100, defaultValue:
            FormBuilder.defaultProps.labelWeight
    },
    labelUppercase: { type: ControlType.Boolean, title: "Label maiúsculo", defaultValue: FormBuilder.defaultProps.labelUppercase },
    labelSpacing: {
        type: ControlType.Number, title: "Espaçamento label", min: 0, max: 8, step: 0.5, defaultValue:
            FormBuilder.defaultProps.labelSpacing
    },
    ...
    inputBorderRadius: {
        type: ControlType.Number, title: "Radius input", min: 0, max: 24, step: 1, defaultValue:
            FormBuilder.defaultProps.inputBorderRadius
    },
    inputPaddingX: {
        type: ControlType.Number, title: "Padding X input", min: 4, max: 32, step: 2, defaultValue:
            FormBuilder.defaultProps.inputPaddingX
    },
    inputPaddingY: {
        type: ControlType.Number, title: "Padding Y input", min: 4, max: 32, step: 2, defaultValue:
            FormBuilder.defaultProps.inputPaddingY
    },
    ...
    buttonRadius: {
        type: ControlType.Number, title: "Radius botão", min: 0, max: 50, step: 1, defaultValue:
            FormBuilder.defaultProps.buttonRadius
    },
    buttonPaddingY: {
        type: ControlType.Number, title: "Padding Y botão", min: 8, max: 32, step: 2, defaultValue:
            FormBuilder.defaultProps.buttonPaddingY
    },
    buttonFontSize: {
        type: ControlType.Number, title: "Fonte botão", min: 12, max: 24, step: 1, defaultValue:
            FormBuilder.defaultProps.buttonFontSize
    },
    buttonFontWeight: {
        type: ControlType.Number, title: "Peso botão", min: 300, max: 900, step: 100, defaultValue:
            FormBuilder.defaultProps.buttonFontWeight
    },
    ...
    labelGap: {
        type: ControlType.Number, title: "Espaço label/input", min: 0, max: 16, step: 1, defaultValue:
            FormBuilder.defaultProps.labelGap
    },
    forceOneColumn: { type: ControlType.Boolean, title: "Forçar 1 coluna", defaultValue: FormBuilder.defaultProps.forceOneColumn },
    mobileBreakpoint: {
        type: ControlType.Number, title: "Breakpoint mobile", min: 320, max: 1024, step: 10, defaultValue:
            FormBuilder.defaultProps.mobileBreakpoint, hidden: (props: any) => props.forceOneColumn
    },
